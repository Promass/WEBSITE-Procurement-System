...
class Display extends Dbh
{
    ...
    {
        $stmt = self::connect()->prepare("SELECT D.did, I.item_name FROM demands D, items I WHERE state = 'pending' AND D.iid = I.iid AND D.did NOT IN (SELECT did FROM quotations WHERE supplier = ?);");
